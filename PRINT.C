...
PSZ pszToolBarClassName	 = (PSZ)"ToolBar";
...
#if defined(INCL_VALAPI)

ValInitialize("Print Example", NULL, VL_VIEWPORT);
...
if ( !WinRegisterClass(hAB, pszPrnSetupClassName, (PFNWP)PrintDriverWndProc, CS_SYNCPAINT | CS_SIZEREDRAW | CS_CLIPCHILDREN, 0UL) )
   return(0);
		       /* Register the window class			*/

if ( !WinRegisterClass(hAB, pszToolBarClassName, (PFNWP)ToolBarWndProc,	CS_SYNCPAINT | CS_SIZEREDRAW, 0UL) )
   return(0);
		       /* Get default printer to use			*/

if ( PrnCreatePrinterList(&prn)	)
   prn.hAB = hAB;
		       /* Determine if DBCS active and perform any	*/
...
if ( !(hwndPrnSetupFrame = CreateStdWindow(HWND_DESKTOP, WS_VISIBLE,
					   ...
					   FCF_DBE_APPSTAT | FCF_SIZEBORDER,
					   pszPrnSetupClassName, (PSZ)"Print Driver", 0L,
					   (HMODULE)NULL, WIN_PRINTDRIVER, &hwndPrnSetup, 6, 5,	203, 120)) )
   return(0);
